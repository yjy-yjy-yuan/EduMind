...
class KnowledgeGraph:
            ...
            try:
                ...
                try:
                    ...
                    self.neo4j_graph.run(
                        "CREATE CONSTRAINT unique_content IF NOT EXISTS FOR (n:Knowledge) REQUIRE n.content IS UNIQUE"
                    )
    ...
    def add_node(self, node_id: str, content: str, source_video: str, timestamp: float) -> Optional[KnowledgeNode]:
        ...
        try:
            ...
            # 创建知识点节点
            node = KnowledgeNode(
                id=node_id, content=content, source_video=source_video, timestamp=timestamp, vector=vector
            )
            ...
            for attempt in range(max_retries):
                try:
                    # 创建Neo4j节点
                    neo4j_node = Node(
                        "Knowledge", id=node_id, content=content, source_video=source_video, timestamp=timestamp
                    )
            ...
            if vector is not None:
                ...
                self.node_embeddings = (
                    np.vstack([self.node_embeddings, vector]) if self.node_embeddings.size else vector.reshape(1, -1)
                )
            ...
            if self.total_nodes % 5 == 0 or self.total_nodes == 1 or self.total_nodes == self.expected_total:
                self.logger.info(
                    f"已处理 {self.total_nodes}/{self.expected_total} 个知识点 | 实体: {self.total_entities} | 关系: {self.total_relations}"
                )
                ...
                if self.neo4j_graph is not None:
                    try:
                        for related_id, similarity in top_related:
                            ...
                            """
                            self.neo4j_graph.run(
                                query, node1_id=node_id, node2_id=related_id, similarity=float(similarity)
                            )
                    except Exception as e:
...
class LearningPathPlanner:
    ...
    def calculate_similarity(self, text1: str, text2: str) -> float:
        ...
        return float(np.dot(vec1, vec2)) / (float(np.linalg.norm(vec1)) * float(np.linalg.norm(vec2)))
