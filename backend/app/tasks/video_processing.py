...
def create_placeholder_preview(video):
    ...
    try:
        ...
        cv2.putText(placeholder_img, "Placeholder Video", (150, 180), cv2.FONT_HERSHEY_SIMPLEX, 1, (100, 100, 100), 2)
    ...
    try:
            ...
            try:
                ...
                if cuda_available:
                    ...
                    warm_cmd = [
                        "python",
                        "-c",
                        "import torch; torch.cuda.init(); torch.cuda.empty_cache(); print('GPU预热完成')",
                    ]
                ...
                try:
                    ...
                    try:
                        ...
                        if is_model_downloaded:
                            raise Exception("模型加载超时,建议使用更小的模型 (如 small 或 base)")
                        else:
                            raise Exception("模型下载超时,请检查网络连接或手动下载模型")
        ...
        try:
            ...
            if retry and self.request.retries < self.max_retries:
                logger.warning(
                    f"⚠️ 任务失败，准备重试 | 尝试: {self.request.retries + 1}/{self.max_retries} | ID: {video_id}"
                )
                # 指数退避策略：每次重试等待时间翻倍
