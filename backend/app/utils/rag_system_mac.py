"""
检索增强生成（RAG）系统 - Mac M系列芯片优化版
功能：
1. 实现检索增强生成（RAG）系统
2. 支持基于字幕的相似内容检索
3. 高效的向量检索和相似度计算
4. 支持 MPS (Metal Performance Shaders) GPU 加速 - 针对 Apple Silicon 优化
...
- 使用正则表达式处理时间戳
- 支持批量处理和增量更新
- 🍎 支持 Apple M1/M2/M3/M4 芯片的 MPS 加速
"""
...
try:
    ...
    # 检测 MPS 可用性（Apple Silicon）
    if torch.backends.mps.is_available():
        print(f"✅ PyTorch可用，MPS加速: 已启用 (Apple Silicon)")
    else:
        print(f"⚠️  PyTorch可用，MPS加速: 未启用 (将使用CPU)")
except ImportError:
    TORCH_AVAILABLE = False
    print("⚠️  未找到PyTorch模块，将使用CPU模式")
...
class RAGSystem:
    ...
    def __init__(self, model_name="all-MiniLM-L6-v2"):
        ...
        # 初始化GPU资源（Mac MPS 不支持 FAISS GPU，所以只使用CPU索引）
        self.use_gpu = False
        self.device = self._get_best_device()

        print(f"🚀 RAG系统初始化完成 | 设备: {self.device}")
    ...
    def _create_index(self, dimension):
        """创建FAISS索引 (Mac版本使用CPU索引)"""
        if not FAISS_AVAILABLE:
            print("⚠️  FAISS模块不可用，无法创建索引")
            return None
        ...
        try:
            ...
            # Mac版本使用CPU索引（使用余弦相似度）
            # FAISS GPU 版本不支持 MPS，所以在 Mac 上使用 CPU 索引
            cpu_index = faiss.IndexFlatIP(dimension)  # 内积用于余弦相似度（向量需要归一化）

            print("✅ 成功创建CPU索引（FAISS在Mac上使用CPU索引）")
            return cpu_index
            ...
            try:
                ...
                if timestamp_end == -1:
                    ...
                    timestamp_match = re.search(
                        r'(\d{2}:\d{2}:\d{2}(?:,\d{3})?)\s*-->\s*(\d{2}:\d{2}:\d{2}(?:,\d{3})?)', p
                    )
                    if timestamp_match:
    ...
    def create_knowledge_base(self, subtitle_path: str) -> 'RAGSystem':
        ...
        try:
            ...
            if TORCH_AVAILABLE and self.device == "mps":
                print(f"🚀 使用 MPS 加速向量计算 (Apple Silicon)")
                vectors = self.model.encode(texts, convert_to_tensor=True, device=self.device)
                # L2归一化，用于余弦相似度
                vectors = torch.nn.functional.normalize(vectors, p=2, dim=1)
                vectors = vectors.cpu().numpy().astype('float32')
            else:
                print(f"💻 使用 CPU 进行向量计算")
            ...
            if FAISS_AVAILABLE:
                ...
                if self.index is not None:
                    ...
                    current_app.logger.warning("⚠️  索引创建失败，无法添加向量")
    ...
    def search_similar_segments(self, query: str, top_k: int = 5) -> List[Dict]:
        ...
        if FAISS_AVAILABLE and self.index is not None:
            ...
            for i, idx in enumerate(indices[0]):
                if idx < len(self.texts) and similarities[0][i] >= min_similarity_threshold:
                    filtered_results.append(
                        {'text': self.texts[idx], 'timestamp': self.timestamps[idx], 'score': float(similarities[0][i])}
                    )
            ...
            if self.vectors is not None:
                ...
                if len(filtered_results) < 2:
                    results = [
                        {'text': self.texts[idx], 'timestamp': self.timestamps[idx], 'score': float(similarities[idx])}
                ...
                for i, text in enumerate(self.texts[:top_k]):
                    results.append({'text': text, 'timestamp': self.timestamps[i], 'score': 0.0})  # 无法计算相似度分数
