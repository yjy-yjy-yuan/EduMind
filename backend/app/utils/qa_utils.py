...
class QASystem:
    ...
    def get_answer_online(
        self,
        question: str,
        api_key: str = "sk-59a6a7690bfb42cd887365795e114002",
        mode: str = 'video',
        context: str = None,
        deep_thinking: bool = False,
    ):
...
请根据提供的视频内容片段回答问题。如果视频内容中没有足够的信息来回答问题，请说明这一点。"""
            ...
            # 格式3: output.text 格式
                ...
                and response.output.text is not None
            ):
...
{final_answer}"""
                    ...
                    if numbered_format and len(numbered_format) >= 3:
                        ...
                        final_answer_match = re.search(
                            r'(?:最终答案[：:]\s*|最后答案[：:]\s*)(.*?)$', final_answer, re.DOTALL
                        )
...
{final_answer}"""
                    ...
                    if analysis_match or thinking_match or solution_match:
                        ...
                        # 提取最终答案
                        final_answer_match = re.search(
                            r'(?:最终答案：|最终答案:|最终答案|最后答案：|最后答案:|最后答案)(.*?)$', answer, re.DOTALL
                        )
                        if final_answer_match:
                            final_answer = final_answer_match.group(1).strip()
                        else:
            ...
            try:
                ...
                if not deep_thinking:
                    ...
                    if response.status_code == 200:
                        try:
                            ...
                            if 'output' in response and 'text' in response.output and response.output.text is not None:
                                ...
                                and 'content' in response.output.choices[0].message
                            ):
                        ...
                        if response.status_code == 200:
                            try:
                                ...
                                if json_obj:
                                    ...
                                    if (
                                        'output' in json_obj
                                        and 'text' in json_obj['output']
                                        and json_obj['output']['text'] is not None
                                    ):
                                        current_text = json_obj['output']['text']
                                    elif (
                                        'output' in json_obj
                                        and 'choices' in json_obj['output']
                                        and json_obj['output']['choices'] is not None
                                        and len(json_obj['output']['choices']) > 0
                                        and 'message' in json_obj['output']['choices'][0]
                                        and 'content' in json_obj['output']['choices'][0]['message']
                                    ):
                                        ...
                                        if 'reasoning_content' in json_obj['output']['choices'][0]['message']:
                                            current_thinking = json_obj['output']['choices'][0]['message'][
                                                'reasoning_content'
                                            ]
                                            if current_thinking:
                                    ...
                                    try:
                                        if 'output' in json_obj and 'finish_reason' in json_obj['output']:
                                            ...
                                            and json_obj['output']['choices'] is not None
                                            and len(json_obj['output']['choices']) > 0
                                            and 'finish_reason' in json_obj['output']['choices'][0]
                                        ):
                    ...
                    if last_complete_response:
                        try:
                            if (
                                'output' in last_complete_response
                                and 'choices' in last_complete_response['output']
                                and last_complete_response['output']['choices'] is not None
                                and len(last_complete_response['output']['choices']) > 0
                                and 'message' in last_complete_response['output']['choices'][0]
                                and 'reasoning_content' in last_complete_response['output']['choices'][0]['message']
                            ):

                                api_thinking_process = last_complete_response['output']['choices'][0]['message'][
                                    'reasoning_content'
                                ]
                                if api_thinking_process and len(api_thinking_process) > 20:
                                    ...
                                    if 'content' in last_complete_response['output']['choices'][0]['message']:
                                        api_content = last_complete_response['output']['choices'][0]['message'][
                                            'content'
                                        ]
                    ...
                    if not thinking_process or len(thinking_process) < 20:
                        logger.warning(
                            f"思考过程为空或太短({len(thinking_process) if thinking_process else 0}个字符)，尝试从完整响应中提取"
                        )
                        ...
                        if think_matches:
                            ...
                            # 方法2：尝试提取"思考过程："部分
                            thinking_match = re.search(
                                r'(思考过程[：:](.*?))(回答[：:]|最终答案[：:]|$)', complete_response, re.DOTALL
                            )
                            if thinking_match:
                                thinking_process = thinking_match.group(2).strip()
                                # 尝试提取最终答案
                                answer_match = re.search(
                                    r'(回答[：:]|最终答案[：:])(.*?)$', complete_response, re.DOTALL
                                )
                                if answer_match:
                                ...
                                # 方法3：尝试提取"分析："部分
                                analysis_match = re.search(
                                    r'(分析[：:](.*?))(解答[：:]|回答[：:]|$)', complete_response, re.DOTALL
                                )
                                if analysis_match:
                                    thinking_process = analysis_match.group(2).strip()
                                    # 尝试提取最终答案
                                    answer_match = re.search(
                                        r'(解答[：:]|回答[：:])(.*?)$', complete_response, re.DOTALL
                                    )
                                    if answer_match:
                                        final_answer = answer_match.group(2).strip()
                                    else:
                                    ...
                                    # 使用一个更简短的通用思考过程
                                    thinking_process = (
                                        f"""我分析了用户的问题"{question}"，并考虑了如何提供最有帮助的回答。"""
                                    )
                    ...
                    if not final_answer or len(final_answer) < 10:
                        final_answer = (
                            complete_response
                            or "你好！我是教育助手，很高兴为您提供帮助。请问有什么我可以帮您解答的问题吗？"
                        )
...
{final_answer}"""
                    ...
                    logger.info(
                        f"返回格式化回答，思考过程长度={len(thinking_process)}，最终答案长度={len(final_answer)}"
                    )
...
{final_answer}"""
                ...
                # 例如：1. **分析问题**：... 2. **思考过程**：... 3. **解决方案**：...
                analysis_match = re.search(
                    r'(?:1\.?\s*\*\*分析问题\*\*：|分析问题：)(.*?)(?:2\.|\*\*思考|$)', complete_response, re.DOTALL
                )
                thinking_match = re.search(
                    r'(?:2\.?\s*\*\*思考过程\*\*：|思考过程：)(.*?)(?:3\.|\*\*解决|$)', complete_response, re.DOTALL
                )
                solution_match = re.search(
                    r'(?:3\.?\s*\*\*解决方案\*\*：|解决方案：)(.*?)(?:最终答案|$)', complete_response, re.DOTALL
                )
                ...
                if analysis_match or thinking_match or solution_match:
                    ...
                    if final_answer_match:
                        ...
                        if re.search(r'分析|思考|解决方案', final_answer):
                            final_answer = "根据以上分析，我的回答是：" + re.sub(
                                r'.*?(您好|你好).*', '您好！我是教育助手，请问有什么可以帮助您的？', complete_response
                            )
...
不要在回答中介绍自己，直接回答问题。"""
            ...
            # 构建请求
            payload = {"model": model_to_use, "prompt": prompt, "system": system_prompt, "stream": False}

            # 发送请求到Ollama API
            response = requests.post(f"{OLLAMA_BASE_URL}/generate", json=payload, timeout=60)  # 设置60秒超时
...
不要在回答中介绍自己，直接回答问题。"""
        ...
        try:
            ...
            try:
                ...
                payload_non_stream = {"model": "qwen3:8b", "prompt": prompt, "system": system_prompt, "stream": False}
...
{final_answer}"""
                                ...
                                # 例如：1. **分析问题**：... 2. **思考过程**：... 3. **解决方案**：...
                                analysis_match = re.search(
                                    r'(?:1\.?\s*\*\*分析问题\*\*：|分析问题：)(.*?)(?:2\.|\*\*思考|$)',
                                    answer,
                                    re.DOTALL,
                                )
                                thinking_match = re.search(
                                    r'(?:2\.?\s*\*\*思考过程\*\*：|思考过程：)(.*?)(?:3\.|\*\*解决|$)',
                                    answer,
                                    re.DOTALL,
                                )
                                solution_match = re.search(
...
{final_answer}"""
            ...
            payload = {"model": "qwen3:8b", "prompt": prompt, "system": system_prompt, "stream": True}
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    if 'response' in data:
                                        ...
                                        if (
                                            deep_thinking
                                            and "<think>" in complete_response
                                            and "</think>" in complete_response
                                        ):
                                            # 检查是否有完整的思考标签对
                                            ...
                                            if think_open_count > 0 and think_open_count == think_close_count:
                                                ...
                                                if think_matches:
                                                    # 合并所有思考过程
                                                    thinking_process = "\n\n".join(
                                                        [match.strip() for match in think_matches]
                                                    )
                                                    # 移除所有思考标签及其内容
                                                    final_answer = re.sub(
