...
class ChatSystem:
    ...
    def chat(
        self, message: str, mode: str = "free", context: Optional[str] = None, rag_system: Optional[RAGSystem] = None
        ...
        try:
            ...
            stream = self.client.chat.completions.create(
                model="qwen-turbo", messages=messages, temperature=0.3, stream=True  # 启用流式输出
            )
    ...
    def _prepare_messages(
        self, message: str, mode: str, context: Optional[str], rag_system: Optional[RAGSystem]
    ) -> List[Dict]:
        """
        准备发送给API的消息列表
        ...
        if mode == "video" and rag_system is not None:
            ...
            try:
                ...
                if similar_segments:
                    rag_context = "\n\n".join(
                        [
                            f"[{seg['timestamp']['start_time']} --> {seg['timestamp']['end_time']}] {seg['text']}"
                    ...
                    messages.append(
                        {
                            "role": "system",
                            "content": "你是一个教育助手。请基于视频内容回答问题。但目前没有找到与问题相关的视频内容，请告知用户。",
                        }
                    )
            except Exception as e:
                ...
                messages.append(
                    {
                        "role": "system",
                        "content": "你是一个教育助手。请基于视频内容回答问题。但检索系统出现错误，请告知用户。",
                    }
                )
        elif mode == "video" and context:
            # 使用提供的上下文
            messages.append(
                {"role": "system", "content": f"你是一个教育助手。请基于以下视频内容回答问题：\n\n{context}"}
            )
        else:
