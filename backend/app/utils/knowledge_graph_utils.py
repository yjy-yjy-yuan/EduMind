...
class KnowledgeGraphManager:
    ...
    def __init__(
        self, uri="bolt://localhost:7687", user="neo4j", password="cjx20040328", similarity_service=None
    ):  # 配置知识图谱
        """初始化知识图谱管理器
        ...
        try:
            ...
            with self.driver.session() as session:
                ...
                for record in result:
                    videos.append({'video_id': record['video_id'], 'title': record['title'], 'tags': record['tags']})
            ...
            with self.driver.session() as session:
                ...
                for source_concept in source_concepts:
                    for target_concept in target_concepts:
                        ...
                        # 计算概念标签的相似度
                        similarity = self.similarity_service.calculate_tag_similarity_with_llm(source_name, target_name)

                        # 如果相似度超过阈值，创建关系
                        if similarity >= similarity_threshold:
                            similar_pairs.append(
                                {'source': source_name, 'target': target_name, 'similarity': similarity}
                            )
        ...
        try:
            ...
            with self.driver.session() as session:
                ...
                if result:
                    ...
                    return {'video_id': result['video_id'], 'title': result['title'], 'tags': result['tags']}
            ...
            with self.driver.session() as session:
                ...
                if is_combined_video:
                    ...
                    for source_video in source_videos:
                        ...
                        logging.info(
                            f"原始视频 {source_id} 有 {len(primary_concepts)} 个一级知识点和 {len(secondary_concepts)} 个二级知识点"
                        )
                    ...
                    logging.info(
                        f"总共找到 {len(relations)} 个关系，其中相似关系 {len(similar_relations)} 个，普通关系 {len(related_relations)} 个"
                    )
                    ...
                    for source_video in source_videos:
                        ...
                        for concept_name in video_data['primary']:
                            ...
                            if concept_id not in node_ids:
                                ...
                                node_data = {
                                    ...
                                    "type": node_type,
                                    "symbolSize": (
                                        40 if is_from_current_selected_video else 35
                                    ),  # 当前视频的知识点稍大一点
                        ...
                        secondary_color = (
                            "#5470c6" if not has_primary_concepts else "#91cc75"
                        )  # 没有一级知识点时使用蓝色，否则使用绿色
                    ...
                    if current_selected_video_id and current_selected_video_id in source_concepts_map:
                        ...
                        for primary_concept, related_concepts in concept_relations.items():
                            for secondary_concept in related_concepts:
                                ...
                                logging.info(
                                    f"添加当前选择视频的二级知识点与一级知识点的关系: {primary_concept} -> {secondary_concept}"
                                )
                    ...
                    for source_id, video_data in source_concepts_map.items():
                        ...
                        for primary_concept in video_data['primary']:
                            ...
                            if concept_id not in node_ids and primary_concept not in all_added_concepts:
                                ...
                                logging.info(
                                    f"添加其他视频的一级知识点与视频节点的连线: video-{video_id} -> {primary_concept}"
                                )
                                ...
                                for node in nodes:
                                    if node["id"] == concept_id:
                                        ...
                                        if concept_source_map.get(primary_concept) != current_selected_video_id:
                                            ...
                                            logging.info(
                                                f"更新知识点 {primary_concept} 的类别和颜色为其他视频的一级知识点"
                                            )
                                        break
                                ...
                                for node in nodes:
                                    if node["id"] == concept_id:
                                        ...
                                        if concept_source_map.get(secondary_concept) != current_selected_video_id:
                                            ...
                                            logging.info(
                                                f"更新知识点 {secondary_concept} 的类别和颜色为其他视频的二级知识点"
                                            )
                                        break
                        ...
                        for primary_concept, related_concepts in concept_relations.items():
                            ...
                            for secondary_concept in related_concepts:
                                ...
                                logging.info(
                                    f"添加其他视频的二级知识点与一级知识点的关系: {primary_concept} -> {secondary_concept}"
                                )
                    ...
                    for relation in relations:
                        ...
                        # 添加连接
                        link_data = {"source": source_id, "target": target_id, "relationship": relation["relationship"]}

                        # 为不同类型的关系设置不同的样式
                        if relation["relationship"] == "SIMILAR_TO":
                            similarity = relation.get("similarity", 0.7)
                            link_data["similarity"] = float(similarity)
                            link_data["lineStyle"] = {"width": 2, "color": "#ee6666", "type": "dashed"}  # 红色
                            logging.info(
                                f"找到相似关系: {relation['source']} <-> {relation['target']} (相似度: {similarity})"
                            )
                        elif relation["relationship"] == "RELATED_TO":
                            link_data["lineStyle"] = {"width": 1.5, "color": "#73c0de"}  # 浅蓝色
                    ...
                    try:
                        ...
                        # 记录日志
                        logging.info(
                            f"视频 {video_id} 有 {len(primary_concepts)} 个一级知识点和 {len(secondary_concepts)} 个二级知识点"
                        )
                    ...
                    for concept_relation in record["concept_relations"]:
                        ...
                        for relation in concept_relation["relations"]:
                            ...
                            # 添加连接
                            links.append(
                                {"source": concept_id, "target": related_id, "relationship": relation["relationship"]}
                            )
    ...
    async def generate_knowledge_graph(
        self, video_id: int, video_title: str, subtitle_text: str, video_tags: List[str] = None
    ) -> bool:
        """生成知识图谱
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "你是一个专业的知识图谱生成助手，擅长从文本中提取核心概念和关系。请严格按照要求的JSON格式返回结果。",
                    },
