...
def get_notes():
    ...
    return jsonify({'status': 'success', 'data': [note.to_dict() for note in notes]})
...
def get_note(note_id):
    ...
    return jsonify({'status': 'success', 'data': note.to_dict()})
...
def generate_content_vector(text):
    ...
    try:
        ...
        tfidf_vectorizer = TfidfVectorizer(
            max_features=100, stop_words=None, min_df=1  # 限制特征数量  # 不使用停用词  # 最小文档频率
        )
...
def create_note():
    ...
    for ts in timestamps:
        timestamp = NoteTimestamp(
            note_id=note.id, time_seconds=ts.get('time_seconds'), subtitle_text=ts.get('subtitle_text')
        )
...
def delete_note(note_id):
    ...
    return jsonify({'status': 'success', 'message': '笔记已删除'})
...
def add_timestamp(note_id):
    ...
    if 'time_seconds' not in data:
        return jsonify({'status': 'error', 'message': '时间点不能为空'}), 400

    # 创建时间戳
    timestamp = NoteTimestamp(
        note_id=note.id, time_seconds=data.get('time_seconds'), subtitle_text=data.get('subtitle_text')
    )

    db.session.add(timestamp)
    db.session.commit()

    return jsonify({'status': 'success', 'data': timestamp.to_dict()})
...
def delete_timestamp(note_id, timestamp_id):
    ...
    return jsonify({'status': 'success', 'message': '时间戳已删除'})
...
def get_tags():
    ...
    return jsonify({'status': 'success', 'data': sorted_tags})
...
def get_similar_notes():
    ...
    return jsonify({'status': 'success', 'data': [note.to_dict() for note in unique_notes]})
...
def batch_delete_notes():
    ...
    if not note_ids:
        return jsonify({'status': 'error', 'message': '未提供笔记ID'}), 400
    ...
    try:
        ...
        return jsonify({'status': 'success', 'message': f'成功删除 {len(notes)} 条笔记', 'deleted_count': len(notes)})
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"批量删除笔记失败: {str(e)}")
        return jsonify({'status': 'error', 'message': f'批量删除笔记失败: {str(e)}'}), 500
...
def batch_export_notes():
    ...
    try:
        ...
        if not notes:
            return jsonify({'status': 'error', 'message': '未找到指定的笔记'}), 404
        ...
        return jsonify({'status': 'error', 'message': f'批量导出笔记失败: {str(e)}'}), 500
...
def export_single_note(note_id):
    ...
    try:
        ...
        response = current_app.response_class(
            md_content, mimetype='text/markdown', headers={'Content-Disposition': f'attachment; filename="{filename}"'}
        )

        return response
    except Exception as e:
        current_app.logger.error(f"导出笔记失败: {str(e)}")
        return jsonify({'status': 'error', 'message': f'导出笔记失败: {str(e)}'}), 500
...
def sync_tags():
    ...
    try:
        ...
        return jsonify({'status': 'success', 'message': '标签数据已同步', 'valid_tags': list(valid_tags)})
    except Exception as e:
        db.session.rollback()
        print(f"同步标签时出错: {str(e)}")
        return jsonify({'status': 'error', 'message': f'同步标签失败: {str(e)}'}), 500
