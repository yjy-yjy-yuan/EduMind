...
def find_similar_videos(video_id):
    ...
    try:
        ...
        for video in all_videos:
            video_data = {'id': video.id, 'title': video.title or video.filename, 'tags': video.tags}
        ...
        similar_videos = llm_similarity_service.find_similar_videos(
            target_tags, all_video_data, threshold=similar_threshold, limit=similar_limit
        )
...
def combine_knowledge_graphs():
    ...
    try:
        ...
        if success:
            return jsonify(
                {'success': True, 'message': f'成功整合视频 {source_video_id} 和视频 {target_video_id} 的知识图谱'}
            )
        else:
            return jsonify({'error': '知识图谱整合失败', 'can_force': True}), 500
...
def check_combined_video(video_id):
    ...
    try:
        ...
        if is_combined:
            return jsonify({'is_combined': True, 'combined_video_id': str(video_id)})

        # 检查是否参与了合并
        combined_video_id = kg_manager.find_combined_video_id(video_id)

        if combined_video_id:
            return jsonify({'is_combined': False, 'is_part_of_combined': True, 'combined_video_id': combined_video_id})
        else:
            return jsonify({'is_combined': False, 'is_part_of_combined': False})
            ...
            for i in range(len(video_ids)):
                for j in range(i + 1, len(video_ids)):
                    ...
                    similarity_results.append(
                        {'source_id': video_ids[i], 'target_id': video_ids[j], 'can_combine': can_combine}
                    )
