...
# 创建知识图谱管理器实例
kg_manager = KnowledgeGraphManager(uri="bolt://localhost:7687", user="neo4j", password="cjx20040328")
                ...
                with open(subtitle_files[0], 'r', encoding='utf-8') as f:
                    ...
                    if isinstance(subtitle_data, list):
                        ...
                        current_app.logger.info(
                            f'视频ID {video_id} 从文件系统找到字幕数据，共 {len(subtitle_texts)} 条'
                        )
    ...
    try:
        ...
        if video_tags and len(video_tags) > 0:
            ...
            if similar_videos:
                ...
                logger.info(
                    f"找到标签相似的视频: {most_similar_video['video_id']} (相似度: {most_similar_video['similarity']})"
                )
                ...
                success = loop.run_until_complete(
                    kg_manager.generate_knowledge_graph(video_id, video_title, subtitle_text, video_tags)
                )
                ...
                if success:
                    ...
                    combine_success = kg_manager.combine_knowledge_graphs(
                        source_video_id=video_id, target_video_id=most_similar_video['video_id']
                    )
        ...
        try:
            ...
            if use_ollama:
                ...
                response = requests.post(
                    ...
                    json={"model": "qwen3:8b", "prompt": prompt, "stream": False, "options": {"temperature": 0.7}},
                    timeout=30,
                )
                ...
                response = client.chat.completions.create(
                    ...
                    messages=[
                        ...
                        {
                            "role": "user",
                            "content": f"基于以下知识点，生成{count}个用户可能想要学习或询问的问题。这些问题应该帮助用户更好地理解这个知识点。只返回问题列表，每个问题应该简洁明了，格式为JSON数组。\n\n知识点: {concept}\n背景信息: {context}",
                        },
                    ],
                    temperature=0.7,
                )
