...
def main():
    ...
    with app.app_context():
        ...
        # 直接使用完整文件名
        subtitle_file_path = os.path.join('backend', 'uploads', 'subtitles', f"{video_filename}.json")

        # 如果文件不存在，尝试使用相对路径
        if not os.path.exists(subtitle_file_path):
            subtitle_file_path = os.path.join('uploads', 'subtitles', f"{video_filename}.json")
