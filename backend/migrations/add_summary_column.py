...
def add_summary_column():
    ...
    try:
        ...
        if 'summary' in column_names:
            logging.info("summary列已存在，跳过迁移")
            conn.close()
            return True

        # 添加summary列
        logging.info("添加summary列到videos表")
        cursor.execute("ALTER TABLE videos ADD COLUMN summary TEXT")

        conn.commit()
        conn.close()

        logging.info("成功添加summary列到videos表")
        return True

    except Exception as e:
        logging.error(f"添加summary列时发生错误: {str(e)}")
        return False
