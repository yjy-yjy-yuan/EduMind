...
摘要格式示例：
"这是一个关于[具体学科/领域]的教学视频，主要讲解了[具体内容/观点/技术/概念]。视频详细介绍了[关键知识点1]、[关键知识点2]和[关键知识点3]等核心内容，适合[目标学习人群]学习使用。"
        ...
        try:
            ...
            payload = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "你是一个专业的教育视频内容分析助手，擅长提炼视频内容要点并生成简洁的摘要。",
                    },
    ...
    def _clean_subtitle_text(self, subtitle_text: str) -> str:
        ...
        if "-->" in subtitle_text:
            ...
            if matches:
                ...
                logger.info(
                    f"正则表达式处理后SRT文本长度: {len(cleaned_text)} 字符，前100个字符: {cleaned_text[:100] if cleaned_text else '空'}"
                )
                return cleaned_text
            ...
            logger.info(
                f"行分割方法处理后SRT文本长度: {len(result)} 字符，前100个字符: {result[:100] if result else '空'}"
            )
            return result
        else:
            # 尝试其他常见字幕格式处理
            ...
            logger.info(
                f"非SRT格式处理后文本长度: {len(text_cleaned)} 字符，前100个字符: {text_cleaned[:100] if text_cleaned else '空'}"
            )
