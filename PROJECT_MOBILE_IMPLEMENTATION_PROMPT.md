...
1. 移动端交互改动（点击、跳转、筛选、按钮状态）验收必须在 iOS 容器 `WKWebView` 内完成，不能仅以浏览器构建成功作为结论。  
2. 每次改动后，必须重新构建 `mobile-frontend` 并确认最新 `dist/` 已同步到 iOS App Web 资源。  
3. 对“点击跳转”类需求，验收必须同时满足：  
   - 页面可点击（触摸可达，不被覆盖层阻断）；  
   - 路由发生变化（如 `/videos?scope=recent|completed|active`）；  
   - 目标页正确渲染对应视频卡片并可进入详情页。  
