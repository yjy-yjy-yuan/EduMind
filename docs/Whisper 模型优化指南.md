...
result = whisper_model.transcribe(
    audio_file,
    language=language,
    fp16=False,  # PyTorch 2.0.x MPS 使用 fp32 更稳定
)
...
### 5. MPS Backend 错误 (Mac 专属)

**错误信息:** `Could not run 'aten::empty.memory_format' with arguments from the 'SparseMPS' backend`

**原因:**
- **PyTorch 2.0.x** 的 MPS backend 存在 SparseMPS bug
- Whisper 模型在加载到 MPS 设备时就会崩溃
- 这是 PyTorch 的已知问题,在 2.1+ 版本中修复

**解决方法:**

✅ **已自动修复** - 代码会自动检测 PyTorch 版本并降级:
```python
...
| PyTorch 版本 | 设备 | 性能 | 稳定性 | 推荐 |
|-------------|------|------|--------|------|
| **2.0.x** | CPU | 基准 | ✅ 稳定 | ✅ 当前使用 |
| **2.0.x** | MPS | ❌ 崩溃 | ❌ 不可用 | ❌ 自动禁用 |
| **2.1+** | MPS | 3-5倍 | ✅ 稳定 | ⭐ 强烈推荐 |

**🔧 推荐升级到 PyTorch 2.1+ (可选):**
```bash
...
**升级后的收益:**
- ✅ **3-5倍加速** - MPS 比 CPU 快得多
- ✅ **稳定性提升** - SparseMPS bug 已修复
- ✅ **自动切换** - 代码会自动使用 MPS
