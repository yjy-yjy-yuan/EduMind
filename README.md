...
1. `../edumind-backend/`（与 EduMind 同级）：真实后端能力与 MySQL 数据写入
...
cp ../edumind-backend/.env.example ../edumind-backend/.env
...
该脚本执行三步：构建 `mobile-frontend` → 从 `../edumind-backend/.env` 读取端口并刷新 iOS 真机后端地址 → 将 `dist/` 同步到 `ios-app/EduMindIOS/EduMindIOS/WebAssets/`。
...
```bash
. .venv/bin/activate
python ../edumind-backend/scripts/init_db.py --info        # 查看表结构
python ../edumind-backend/scripts/init_db.py --create      # 补创建缺失表（不删已有数据）
python ../edumind-backend/scripts/init_db.py --reset       # 删除并重建（危险）
python ../edumind-backend/scripts/init_db.py --emit-sql ../edumind-backend/scripts/mysql_managed_schema.sql
```

导出的 SQL 文件：[`../edumind-backend/scripts/mysql_managed_schema.sql`](../edumind-backend/scripts/mysql_managed_schema.sql)，可直接在 Navicat 执行。

## 用户认证

- 注册接受邮箱或手机号（至少一项），密码要求大小写字母 + 数字 + 特殊字符（≥8 位）。
- 登录只接受邮箱/手机号 + 密码；登录后支持修改用户名（写回 `users.username`）和上传头像（存于 `../edumind-backend/uploads/avatars/`）。
...
| 文件 | 说明 |
|---|---|
| `../edumind-backend/app/services/search/chunker.py` | 视频切片（按时长自动调整粒度） |
| `../edumind-backend/app/services/search/embedder.py` | 本地文本向量嵌入（`LocalEmbedder`） |
| `../edumind-backend/app/services/search/store.py` | ChromaDB 持久化 |
| `../edumind-backend/app/services/search/search.py` | 搜索编排 |
| `../edumind-backend/app/routers/search.py` | 搜索接口 |
| `../edumind-backend/app/tasks/vector_indexing.py` | 后台索引任务 |

当前能力：字幕语义搜索 + 相似度排序 + `preview_text` 回填；视频处理完成后按配置自动提交索引。

当前限制：搜索路由优先取 `X-User-ID` 请求头（未完全接入认证体系）；iOS / H5 搜索界面为轻量版本，结果摘要与完整认证联动尚未补齐。

部署与限制详情：[`../edumind-backend/SEMANTIC_SEARCH_DEPLOYMENT.md`](../edumind-backend/SEMANTIC_SEARCH_DEPLOYMENT.md)。
...
`../edumind-backend/` 统一代理 Sleek API，前端与 iOS 容器不直接暴露 `SLEEK_API_KEY`。
...
启用：在 `../edumind-backend/.env` 中填写 `SLEEK_API_KEY`（scope 至少含 `projects:read/write`、`components:read`、`chats:read/write`、`screenshots`）。

## 后端测试目录

```
../edumind-backend/tests/
├── unit/        # 单元测试
├── api/         # 接口测试
├── smoke/       # 冒烟测试（CI 门禁）
└── integration/ # 集成测试
...
- [`../edumind-backend/README.md`](../edumind-backend/README.md)
- [`../edumind-backend/README_RUN.md`](../edumind-backend/README_RUN.md)
